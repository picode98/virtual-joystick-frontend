...
Public Class vJoyControl
    ...
    Private Function isValidController()
        ...
        If (Not (joystick.GetVJDButtonNumber(deviceID) = 11) Or Not (joystick.GetVJDAxisExist(deviceID, HID_USAGES.HID_USAGE_X)) Or Not (joystick.GetVJDAxisExist(deviceID, HID_USAGES.HID_USAGE_Y)) _
              Or Not (joystick.GetVJDAxisExist(deviceID, HID_USAGES.HID_USAGE_Z))) Then
            returnValue = False
        End If
    ...
    Public Sub UpdateJoystickData(newControllerData As Form1.controllerData, oldControllerData As Form1.controllerData, sendAll As Boolean)
        'Dim POVUpdateFailed As Boolean = False
        Dim UpdateFailed As Boolean = False

        If sendAll Or Not (newControllerData.joystickPosition = oldControllerData.joystickPosition) Then
            joystickData.AxisX = Math.Round(327.67 * (newControllerData.joystickPosition.X / 2.0))
            joystickData.AxisY = Math.Round(327.67 * (newControllerData.joystickPosition.Y / 2.0))
        End If

        If sendAll Or Not (newControllerData.throttlePosition = oldControllerData.throttlePosition) Then
            joystickData.AxisZ = Math.Round(327.67 * (newControllerData.throttlePosition))
        End If
